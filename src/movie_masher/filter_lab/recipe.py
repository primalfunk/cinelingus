...
from movie_masher.util import read_json, utc_now, write_json
...
def load_recipe(path: Path, *, registry: FilterRegistry | None = None) -> RecipeLoadResult:
    ...
    recipe = FilterRecipe(
        ...
        target_duration=float(data["target_duration"]) if data.get("target_duration") is not None else None,
