...
def _event_mapping_coverage(turns: list[dict[str, Any]], speech_items: list[dict[str, Any]]) -> float:
    if not speech_items:
        return 0.0
    mapped = {str(row.get("source_id")) for row in turns if row.get("valid") and row.get("source_id")}
    return round(sum(1 for item in speech_items if str(item.get("id")) in mapped) / len(speech_items), 4)
