...
from .gui_implementation import *  # noqa: F401,F403 - preserve the public GUI module surface
...
class CinelingusInstrumentApp(_InstrumentImplementation):
    """Instrument shell with reliable selection and a visible contract lifecycle."""

    def __init__(self) -> None:
        super().__init__()
        self.default_input_dir = default_input_directory()
        self.last_input_dir: Path | None = None
        # Committed configuration paths remain available to the CLI, but a GUI launch
        # must not present them as material the operator actually selected.
    ...
    def _start_run(self) -> None:
        ...
        summary = (
            f"Input contract ready. Canonical extent {duration}{curtailed}. "
            f"Prior evidence: {evidence_state}."
        )
