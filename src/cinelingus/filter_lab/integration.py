...
def build_strategy_schedule(
    ...
    *,
    ...
    placement_qualification: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    eligible = placement_windows if placement_windows is not None else windows
    if placement_windows is not None and not eligible:
        detail = dict(placement_qualification or {})
        raise ValueError(
            f"{filter_id} exhausted preferred whole-moment and local complete-shot placement qualification: "
            f"{detail.get('final_legal_authored_placements', 0)} legal placements from "
            f"{detail.get('dialogue_windows_examined', len(windows))} dialogue windows, "
            f"{detail.get('preferred_audio_qualified_moments', 0)} preferred moments, and "
            f"{detail.get('complete_shot_submoments_constructed', 0)} constructed submoments."
        )
    if spec.uses_full_window_context:
        schedule = spec.builder(
            clips=clips,
            windows=windows,
            placement_windows=eligible,
            duration=duration,
            parameters=parameters,
            seed=seed,
        )
    else:
        schedule = spec.builder(clips=clips, windows=eligible, duration=duration, parameters=parameters, seed=seed)
    schedule["montage_window_eligibility"] = {
        "analysis_window_count": len(windows),
        "placement_window_count": len(eligible),
        "strategy_used_full_window_context": spec.uses_full_window_context,
        "authored_placements_restricted_to_audio_safe_moments": placement_windows is not None,
    }
    if placement_qualification is not None:
        schedule["placement_qualification"] = dict(placement_qualification)
...
def write_filter_artifacts(
    *,
    ...
    if output_form != "full_length" or target_duration is not None:
        raise ValueError("Filter artifacts support full-length output only; target-duration output was removed.")
