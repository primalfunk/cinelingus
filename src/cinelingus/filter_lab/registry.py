...
class FilterRegistry:
    ...
    def get_in_family(self, family_id: str, filter_name_or_id: str) -> FilterDefinition:
        ...
        for definition in self.filters_for_family(family_id):
            accepted = {self._key(definition.id), self._key(definition.name), *(self._key(alias) for alias in definition.legacy_aliases)}
            if key in accepted:
                return definition
    ...
    def validate_stack(self, filter_ids: list[str] | tuple[str, ...]) -> list[dict[str, str]]:
        ...
        from .combination import compile_ordered_combination

        decisions: list[dict[str, str]] = []
        for predecessor, successor in zip(definitions, definitions[1:]):
            ...
            if not decision.executable:
                detail = "; ".join(decision.reasons) or "combination certification is absent"
                raise ValueError(
                    f"Filter combination {decision.pair_id} is {decision.status.value} and cannot execute: {detail}"
                )
            decisions.append(decision.recipe_record())
        return decisions
...
def _definition(
    ...
    *,
    ...
    return FilterDefinition(
        ...
        supported_output_forms=("full_length",) if implemented else (),
...
DEFINITIONS = (
    ...
    _definition(
        "multiworld.translation", "Translation", "multiworld",
        ...
        parameters=(INTENSITY, PERFORMANCE), implemented=True, implementation_key="translation", implementation_class="A", execution_mode="transformation", aliases=("translation", "multiworld.translation", "Transposition"), preview=True,
        preserves={"time": "Anchor chronology preserved", "performance": "Matched where possible"},
        minimum_films=2, maximum_films=2, cinematic_law="Translation",
    ...
    *tuple(_catalog_definition(
    ...
    _multiworld_dialogue_definition(
        "echo_chamber", "Echo Chamber", "Translation",
