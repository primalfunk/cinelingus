...
class StrategySpec:
    ...
    uses_full_window_context: bool = False
...
def scheduling_strategy(
    ...
    *,
    ...
    def register(builder: Callable[..., dict[str, Any]]):
        ...
        STRATEGY_SPECS[filter_id] = StrategySpec(
            builder=builder,
            progress_stages=progress_stages,
            uses_full_window_context=uses_full_window_context,
        )
        return builder
    return register
...
@scheduling_strategy(
    "contagion",
    ("building speaker contact graph", "simulating exposure", "propagating infection", "generating infection timeline"),
    uses_full_window_context=True,
)
def build_contagion_schedule(
    *, clips: list[dict[str, Any]], windows: list[dict[str, Any]], duration: float,
    parameters: dict[str, Any], seed: int = 1,
    placement_windows: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
    rng = random.Random(seed)
    clip_pools = _speaker_pools(clips)
    ...
    authored_windows = placement_windows if placement_windows is not None else windows
    used: set[str] = set()
    mappings: list[dict[str, Any]] = []
    rejected: list[dict[str, Any]] = []
    transformed_by_speaker: Counter[str] = Counter()
    rescue_used = False
    for index, window in enumerate(sorted(authored_windows, key=_start)):
    ...
    if not mappings and authored_windows:
        for window in sorted(authored_windows, key=lambda row: (_start(row), _id(row))):
            target = _speaker(window)
            state = infected.get(target)
            if target == carrier or state is None or _start(window) < float(state["infection_time"]):
                continue
            ...
            if not candidates:
                continue
            candidates.sort(
                key=lambda clip: (_duration_fit(_duration(clip), _duration(window)), _id(clip)),
                reverse=True,
            )
            chosen = candidates[0]
            used.add(_id(chosen))
            progress = min(1.0, max(0.0, _start(window) / max(duration, 0.001)))
            strength = min(1.0, ratio * (0.35 + 0.65 * progress))
            mapping = _mapping(chosen, window, "contagion", "audio_safe_montage_rescue")
            mapping.update({
                "infection_time": round(float(state["infection_time"]), 3),
                "infection_source": state["infecting_speaker"],
                "infection_strength": round(strength, 4),
                "progression_value": round(strength, 4),
                "montage_rescue": True,
            })
            mappings.append(mapping)
            transformed_by_speaker[target] += 1
    ...
    metrics = {
        ...
        "montage_rescue_used": rescue_used,
    }
    ...
    if rescue_used:
        schedule["filter_fallbacks"] = [{
            "reason": "SEEDED_INTENSITY_SKIPPED_ALL_AUDIO_SAFE_POST_INFECTION_WINDOWS",
            "action": "SELECTED_DETERMINISTIC_AUDIO_SAFE_POST_INFECTION_WINDOW",
        }]
    return schedule
...
def _mapping(clip: dict[str, Any], window: dict[str, Any], operation: str, reason: str) -> dict[str, Any]:
    ...
    if window.get("montage_moment_id") is not None:
        mapping["montage_moment_id"] = window.get("montage_moment_id")
        mapping["montage_audio_eligible"] = bool(window.get("montage_audio_eligible"))
        for key in (
            "montage_parent_moment_id",
            "montage_qualification_stage",
            "montage_eligibility_reason",
            "montage_submoment_start",
            "montage_submoment_end",
            "montage_submoment_shot_ids",
        ):
            if window.get(key) is not None:
                mapping[key] = window.get(key)
    return mapping
