...
def build_run_report(
    *,
    ...
    for role, diagnostics in (("source", speaker_summary.get("source_diagnostics", {})), ("destination", speaker_summary.get("destination_diagnostics", {}))):
        if diagnostics.get('diarization_status') == 'FALLBACK' or (
            not diagnostics.get('diarization_status') and diagnostics.get('fallback_used')
        ):
        ...
        elif int(diagnostics.get('inferred_item_count', 0) or 0):
            warnings.append(
                '{} speaker alignment used estimates for {} / {} items'.format(
                    role,
                    diagnostics.get('inferred_item_count', 0),
                    diagnostics.get('speech_item_count', 0),
                )
            )
...
def _format_text_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "Cinelingus Translation Run Report",
        "===========================================",
    ...
    if speakers:
        ...
        lines.extend([
            ...
            f"  source direct speaker evidence: {source_diag.get('direct_item_count', 0)} / {source_diag.get('speech_item_count', 0)} items",
            f"  source speaker inference: {source_diag.get('fallback_status', 'unknown')}",
            ...
            f"  destination direct speaker evidence: {destination_diag.get('direct_item_count', 0)} / {destination_diag.get('speech_item_count', 0)} items",
            f"  destination speaker inference: {destination_diag.get('fallback_status', 'unknown')}",
